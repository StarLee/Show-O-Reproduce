...
class Text2ImageDataset:
    def __init__(
        ...
        if external_caption_path != '':
            # 提前加载所有caption到内存中
            with open(external_caption_path, 'r') as external_caption_file:
                self.external_caption_json = json.load(external_caption_file)
            processing_pipeline = [
    ...
    def load_external_caption(self, sample):
        ...
        if 'sa1b' in sample['__url__']:
            #captionf = f"{self.external_caption_path}/{sample['__key__'].split('/')[-1]}.txt"
            captions = self.external_caption_json.get(sample['__key__'].split('/')[-1], '')
